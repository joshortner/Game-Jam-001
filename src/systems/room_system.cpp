...
namespace bk
{
    ...
    void room_system::on_update(double dt, flecs::world& world)
    {
        sf::Vector2f room_coordinates = sf::Vector2f(
            m_player.get<component::transform>()->position.x / 640.f, // room_size.x
            m_player.get<component::transform>()->position.y / 360.f  // room_size.y
        );
        ...
        for (uint32_t i = 0; i < SIZE; i++)
            if (indices[i] == -1)
                ...
                .set(component::transform {
                    .position = { 640.f * adjacent[i].x, 360.f * adjacent[i].y },
                    .scale    = { 1, 1 },
                    .rotation = 0
                })
